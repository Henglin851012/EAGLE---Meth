...
void set_prob_matrix(double *matrix, const read_t *read, const double *is_match, const double *no_match, const int *seqnt_map, const int bisulfite)
{
    ...
    {
        for (i = 0; i < NT_CODES; i++)
            matrix[read->length * i + b] = no_match[b];
        ...
        if (bisulfite > 0)
        {
            ...
            if ((bisulfite == 1) && (read->qseq[b] == 'T') && ((!read->is_read2 && !read->is_reverse) || (read->is_read2 && read->is_reverse)))
                matrix[read->length * seqnt_map['C' - 'A'] + b] = is_match[b]; // unmethylated forward strand, top strand
            else if ((bisulfite == 2) && (read->qseq[b] == 'A') && ((!read->is_read2 && read->is_reverse) || (read->is_read2 && !read->is_reverse)))
                matrix[read->length * seqnt_map['G' - 'A'] + b] = is_match[b]; // unmethylated reverse strand, bottom strand
            else if ((bisulfite >= 3) && (read->qseq[b] == 'T') && ((!read->is_read2 && !read->is_reverse) || (read->is_read2 && read->is_reverse)))
                matrix[read->length * seqnt_map['C' - 'A'] + b] = is_match[b]; // unmethylated forward strand, top strand
            else if ((bisulfite >= 3) && (read->qseq[b] == 'A') && ((!read->is_read2 && read->is_reverse) || (read->is_read2 && !read->is_reverse)))
                matrix[read->length * seqnt_map['G' - 'A'] + b] = is_match[b]; // unmethylated reverse strand, bottom strand
        }
    }
}

/* calculate methlyation probability */
double calc_read_prob_meth(const double *matrix, int read_length, const char *seq, int seq_length, int pos, int *seqnt_map)
{
    ...
    for (i = pos; i < end; i++)
    {
        ...
        {
            exit_err("Character %c at pos %d (%d) not in valid alphabet\n", seq[i], i, seq_length);
        }
...
double calc_prob(const double *matrix, int read_length, const char *seq, int seq_length, int pos, int *splice_pos, int *splice_offset, int n_splice, int *seqnt_map)
{
    ...
    { // calculate the probability for each splice section separately
        int r_pos = 0;
        int g_pos = pos;
        for (i = 0; i <= n_splice; i++)
        {
            ...
            for (j = 0; j < NT_CODES; j++)
                memcpy(&submatrix[r_len * j], &matrix[read_length * j + r_pos], r_len * sizeof(double));
...
double x_drop(const double *matrix, int read_length, const char *seq, int seq_length, int start, int end, int gap_op, int gap_ex, int *seqnt_map)
{ /* short in long version */
    ...
    do
    {
        ...
        for (i = L; i <= U + 1; i++)
        {
            ...
            S[curr_i] = upleft;
            if (A[curr_i] > S[curr_i])
                S[curr_i] = A[curr_i];
            if (B[curr_i] > S[curr_i])
                S[curr_i] = B[curr_i];
            if (S[curr_i] > t_max)
                t_max = S[curr_i];
            else if (S[curr_i] < max_score - x_drop_score)
                S[curr_i] = -INFINITY;
...
{ /* short in long version */
    ...
    for (i = start; i <= end; i++)
    {
        ...
        for (j = 1; j <= read_length; j++)
        {
            ...
            curr[j] = upleft;
            if (a_gap_curr[j] >= curr[j])
                curr[j] = a_gap_curr[j];
            if (b_gap_curr[j] >= curr[j])
                curr[j] = b_gap_curr[j];
            if (curr[j] > row_max)
                row_max = curr[j];
        }
    ...
    for (i = start; i < end; i++)
    {
        ...
        for (j = 0; j < combo->len; j++)
        {
            ...
            for (m = 0; m < l; m++)
            {
                ...
                if (r_pos < 0)
                    continue;
                if (r_pos >= read_length || g_pos >= seq_length)
                    break;
                ...
                if (m >= alt_len)
                {
                    if (g_pos + ref_len - alt_len >= seq_length)
                        break;
                ...
                if (x < 0 || x > 57 || (x > 25 && x < 32))
                {
                    exit_err("Ref character %c at gpos %d (%d) not in valid alphabet\n", seq[g_pos], g_pos, seq_length);
                }
                if (y < 0 || y > 57 || (y > 25 && y < 32))
                {
                    exit_err("Alt character %c at rpos %d for %s;%d;%s;%s not in valid alphabet\n", v->alt[m], m, v->chr, v->pos, v->ref, v->alt);
                }
...
/* calculate methlyation probability */
double calc_prob_region_meth(const double *matrix, int read_length, const char *seq, int seq_length, int pos, int start, int end, int *seqnt_map)
{
    if (start < 0)
        start = 0;
    else if (start >= seq_length)
    ...
    for (i = start; i < end; i++)
    {
        p[i - start] = calc_read_prob_meth(matrix, read_length, seq, seq_length, i, seqnt_map);
    }
    return log_sum_exp(p, end - start);
}

double calc_prob_meth(const double *matrix, int read_length, const char *seq, int seq_length, int pos, int *splice_pos, int *splice_offset, int n_splice, int *seqnt_map)
{
    /* Get the sequence g in G and its neighborhood (half a read length flanking regions) */
    int start = pos - (read_length / 2);
    int end = pos + (read_length / 2);
    ...
    if (n_splice == 0)
    {
        probability = calc_prob_region_meth(matrix, read_length, seq, seq_length, pos, start, end, seqnt_map);
    }
    else
    { // calculate the probability for each splice section separately
        int r_pos = 0;
        ...
        for (i = 0; i <= n_splice; i++)
        {
            ...
            for (j = 0; j < NT_CODES; j++)
                memcpy(&submatrix[r_len * j], &matrix[read_length * j + r_pos], r_len * sizeof(double));
            probability += calc_prob_region_meth(submatrix, r_len, seq, seq_length, g_pos, start, end, seqnt_map);
            free(submatrix);
            submatrix = NULL;
...
// void calc_prob_region_meth2(double *prgu, double *prgv, vector_int_t *combo, variant_t **var_data, double *matrix, int read_length, const char *seq, int seq_length, int pos, int start, int end, int *seqnt_map, int rev)
// {
//     if (start < 0)
//         start = 0;
//     else if (start >= seq_length)
...
//     int i, j, m;
//     double prgu_i[end - start], prgv_i[end - start], max;
//     for (i = start; i < end; i++)
//     {
//         int n = i - start;
//         prgu_i[n] = calc_read_prob_meth(matrix, read_length, seq, seq_length, i, seqnt_map); // reference probability per position i
//         prgv_i[n] = prgu_i[n];                                                               // alternative probability per position i
//         max = prgu_i[n];
//         int offset = 0;
//         for (j = 0; j < combo->len; j++)
...
//                     if (g_pos + ref_len - alt_len >= seq_length)
//                         break;
//                     if (seq[g_pos + ref_len - alt_len] == 'M')
//                     {
//                         y = 'c' - 'A';
//                     }
//                     else if (seq[g_pos + ref_len - alt_len] == 'W')
//                     {
//                         y = 'g' - 'A';
//                     }
//                     else
...
// void calc_prob_meth2(double *prgu, double *prgv, vector_int_t *combo, variant_t **var_data, double *matrix, int read_length, const char *seq, int seq_length, int pos, int *splice_pos, int *splice_offset, int n_splice, int *seqnt_map, int rev)
// {
//     /* Get the sequence g in G and its neighborhood (half a read length flanking regions) */
//     int start = pos - (read_length / 2);
//     int end = pos + (read_length / 2);
...
//     if (n_splice == 0)
//     {
//         calc_prob_region_meth2(prgu, prgv, combo, var_data, matrix, read_length, seq, seq_length, pos, start, end, seqnt_map, rev);
//     }
//     else
//     { // calculate the probability for each splice section separately
//         int r_pos = 0;
...
//             for (j = 0; j < NT_CODES; j++)
//                 memcpy(&submatrix[r_len * j], &matrix[read_length * j + r_pos], r_len * sizeof(double));
//             calc_prob_region_meth2(prgu, prgv, combo, var_data, submatrix, r_len, seq, seq_length, pos, start, end, seqnt_map, rev);
//             free(submatrix);
//             submatrix = NULL;
