...
/* meth_eagle */
#define METH_EAGLE 1
char *modify_ref = NULL; // the-highest-likelihood-variant-modified Ref version
int modify_ref_length;
int modify_already = 0;
char *meth_ref = NULL;
int meth_ref_length;
char *or_ref = NULL;
int or_ref_length;
int max_var_set_len = 0;
int test = 0;
int meth_set_size = 30;
int overlap = 5;
/* Time info */
static time_t now;
...
static char *construct_altseq(const char *refseq, int refseq_length, const vector_int_t *combo, variant_t **var_data, int *altseq_length)
{
    ...
    *altseq_length = refseq_length;
    for (i = 0; i < combo->len; i++)
    {
        ...
        // fprintf(stderr,"vardata %d:%d\t%s\t%s\n",i,v->pos,v->ref,v->alt);
...
static inline void variant_print(char **output, const vector_t *var_set, int i, int nreads, int not_alt_count, int has_alt_count, double total, double has_alt, double not_alt)
{
    ...
    int n = snprintf(NULL, 0, "%s\t%d\t%s\t%s\t%d\t%d\t%d\t%f\t%f\t", var_data[i]->chr, var_data[i]->pos, var_data[i]->ref, var_data[i]->alt, nreads, not_alt_count, has_alt_count, exp(prob), odds) + 1;
    char token[n];
    snprintf(token, n, "%s\t%d\t%s\t%s\t%d\t%d\t%d\t%f\t%f\t", var_data[i]->chr, var_data[i]->pos, var_data[i]->ref, var_data[i]->alt, nreads, not_alt_count, has_alt_count, exp(prob), odds);
...
static void calc_likelihood(stats_t *stat, vector_t *var_set, const char *refseq, const int refseq_length, read_t **read_data, const int nreads, int seti, int *seqnt_map)
{
    ...
    if (!lowmem)
    {
        for (i = 0; i < stat->combo->len; i++)
        {
            ...
            // print_status("var_data %d: %s\t%d\t%s\t%s\t\n",i,v->chr,v->pos,v->ref,v->alt);
        }
    }
    ...
    for (readi = 0; readi < nreads; readi++)
    {
        ...
        stat->seen++;
        // print_status("name:%s\t len:%d\t r,f:%d\t splice:%d\n",read_data[readi]->name,read_data[readi]->length,read_data[readi]->is_reverse, read_data[readi]->n_splice);
        ...
        /*  Paralog Outside Exact Formuation: Probability that read is from an outside the reference paralogous "elsewhere", f in F.  Approximate the bulk of probability distribution P(r|f):
        ...
        else
        {
            ...
            // fprintf(stderr, "pos: %d\fstart: %d\fend: %d\n", read_data[readi]->pos, read_data[readi]->n_splice, read_data[readi]->n_splice);
        }
        //printf("%f\t%f\n\n", prgv, prgu);
...
/*---- todo: calculate methylation likelihood ----*/
static char *construct_me_altseq(const char *refseq, int refseq_length, const vector_int_t *combo, variant_t **var_data, int *altseq_length, int i)
{
    int offset = 0;
    char *altseq = strdup(refseq);
    *altseq_length = refseq_length;
    ...
    { // snps, equal length haplotypes
        memcpy(altseq + pos, var_alt, var_alt_length * sizeof(*var_alt));
        // memcpy(altseq + pos, var_ref, var_ref_length * sizeof(*var_ref));
    }
    else
    { // indels
        char *newalt = malloc((*altseq_length + delta + 1) * sizeof(*newalt));
...
static void meth_calc_likelihood(stats_t *stat, vector_t *var_set, const char *refseq1, const int refseq1_length, const char *refseq2, const int refseq2_length, read_t **read_data, const int nreads, int seti, int *seqnt_map, int combo_num)
{
    size_t i, readi;
    stat->ref = 0;
    stat->alt = 0;
    ...
    if (has_indel || dp)
    {
        altseq = construct_altseq(refseq1, refseq1_length, stat->combo, var_data, &altseq_length);
        // fprintf(stderr, "####\n");
    }
    ...
    for (readi = 0; readi < nreads; readi++)
    {
        ...
        stat->seen++;
        // print_status("name:%s\t len:%d\t r,f:%d\t splice:%d\t%d\t%d\n", read_data[readi]->name, read_data[readi]->length, read_data[readi]->is_reverse, read_data[readi]->n_splice, read_data[readi]->is_read2, read_data[readi]->pos);
        double is_match[read_data[readi]->length], no_match[read_data[readi]->length];
        for (i = 0; i < read_data[readi]->length; i++)
        {
            is_match[i] = p_match[read_data[readi]->qual[i]];
        ...
        // else
        // {
        //     calc_prob_meth(&prgu, &prgv, stat->combo, var_data, readprobmatrix, read_data[readi]->length, refseq, refseq_length, read_data[readi]->pos, read_data[readi]->splice_pos, read_data[readi]->splice_offset, read_data[readi]->n_splice, seqnt_map, read_data[readi]->is_reverse);
        // }

        /*test*/
        prgu = calc_prob_meth(readprobmatrix, read_data[readi]->length, refseq1, refseq1_length, read_data[readi]->pos, read_data[readi]->splice_pos, read_data[readi]->splice_offset, read_data[readi]->n_splice, seqnt_map);
        double max = -1000;
        for (int seti = 0; seti < stat->combo->len; seti++)
        {
            // for (int i = 0; i < combo_num; i++)
            // {
            altseq = construct_me_altseq(refseq2, refseq2_length, stat->combo, var_data, &altseq_length, seti);
            // altseq = construct_me_altseq(refseq1, refseq1_length, stat->combo, var_data, &altseq_length, seti);
            prgv = calc_prob_meth(readprobmatrix, read_data[readi]->length, altseq, altseq_length, read_data[readi]->pos, read_data[readi]->splice_pos, read_data[readi]->splice_offset, read_data[readi]->n_splice, seqnt_map);
            // fprintf(stderr, "altseq:\n%s\n", altseq);
            // fprintf(stderr, " prgv:%f\n", prgv);
            if (max < prgv)
            {
                max = prgv;
            }
        ...
        prgv = max;
        // fprintf(stderr, "prgv:%f\n", prgv);
        // prgu = calc_prob_meth(readprobmatrix, read_data[readi]->length, refseq, refseq_length, read_data[readi]->pos, read_data[readi]->splice_pos, read_data[readi]->splice_offset, read_data[readi]->n_splice, seqnt_map, rev);
        ...
        if (prgv > read_data[readi]->prgv)
        {
            // fprintf(stderr, "#prgv : %f\t#read_prgv : %f\n", prgv, read_data[readi]->prgv);
            read_data[readi]->index = seti;
            read_data[readi]->prgu = (float)prgu;
            read_data[readi]->prgv = (float)prgv;
            read_data[readi]->pout = (float)pout;
...
/*-------------todo : meth_evaluate---------------*/
static char *meth_evaluate(vector_t *var_set)
{
    size_t i, readi, seti;
    ...
    /* Reference sequence */
    char *refseq1, *refseq2;
    refseq1 = meth_ref;
    refseq2 = modify_ref;
    int refseq1_length = meth_ref_length;
    int refseq2_length = modify_ref_length;
    ...
    // if (max_var_set_len < (variant_t *)var_set->len)
    // {
    //     max_var_set_len = (variant_t *)var_set->len;
    // }
    // fprintf(stderr,"var set len : %d\n",(variant_t *)var_set->len);
    // for (i = 0; i < (variant_t *)var_set->len; i++)
    // { //Print var_sets
    //     fprintf(stderr, "%s\t%d\t%s,%s\n", var_data[i]->chr, var_data[i]->pos, var_data[i]->ref, var_data[i]->alt);
    // }
    // fprintf(stderr, "\n");
    // for (seti = 0; seti < combo->len; seti++)
    // { // Print combinations
    //     fprintf(stderr, "%d\t", (int)seti);
    //     for (i = 0; i < ((vector_int_t *)combo->data[seti])->len; i++)
    //     {
    //         fprintf(stderr, "%d;", ((vector_int_t *)combo->data[seti])->data[i]);
    //     }
    //     fprintf(stderr, "\t");
    //     for (i = 0; i < ((vector_int_t *)combo->data[seti])->len; i++)
    //     {
    //         variant_t *v = var_data[((vector_int_t *)combo->data[seti])->data[i]];
    //         // int len = strlen(v->ref);
    //         fprintf(stderr, "%s,%d,%s,%s;", v->chr, v->pos, v->ref, v->alt);
    ...
    /*caculate each combo->[seti] likelihood, each combo->[seti]->contain one different CpG methlyation*/
    for (seti = 0; seti < combo->len; seti++)
    { // all, singles
        int combo_num = ((vector_int_t *)combo->data[seti])->len;
        stats_t *s = stats_create((vector_int_t *)combo->data[seti], read_list->len);
        meth_calc_likelihood(s, var_set, refseq1, refseq1_length, refseq2, refseq2_length, read_data, read_list->len, seti, seqnt_map, combo_num);
        vector_add(stats, s);
    }
    if (var_set->len > 1)
    { // doubles and beyond
        ...
        while (s = heap_pop(h), s != NULL)
        {
            ...
            for (i = 1; i < c->len; i++)
            {
                ...
                int combo_num = ((vector_int_t *)combo->data[i])->len;
                meth_calc_likelihood(s, var_set, refseq1, refseq1_length, refseq2, refseq2_length, read_data, read_list->len, stats->len, seqnt_map, combo_num);
                vector_add(stats, s);
                heap_push(h, s->mut, s);
            }
    ...
    { /* Max likelihood variant hypothesis */
        ...
        for (i = 0; i < stat[max_seti]->combo->len; i++)
        {
            ...
            // print_status("chr %s\tpos %d\tref %s\talt %s\n", var_data[stat[max_seti]->combo->data[i]]->chr,
            //                                          var_data[stat[max_seti]->combo->data[i]]->pos,
            //                                          var_data[stat[max_seti]->combo->data[i]]->ref,
            //                                          var_data[stat[max_seti]->combo->data[i]]->alt);
        }
...
/*--------------todo : meth_poool-----------------*/
static void *meth_pool(void *work)
{
    print_status("enter pool_meth()\n");
    ...
    { //pthread_t ptid = pthread_self(); uint64_t threadid = 0; memcpy(&threadid, &ptid, min(sizeof (threadid), sizeof (ptid)));
        ...
        if (var_set == NULL)
        {
            print_status("var set is NULL");
            break;
        }
        // print_status("%d\n",var_set->len);
        char *outstr = meth_evaluate(var_set);
...
/*--------------todo : meth_process --------------*/
static void meth_process(const vector_t *var_list, FILE *out_fh)
{
    print_status("enter process_meth()\n");
    size_t i, j;
    ...
    { /* Variants that are close together as sets */ //todo:  how to cut methlyation set
        while (i < var_list->len)
        {
            vector_t *curr = vector_create(8, VARIANT_T);
            ...
            j = i;
            for (int tmp = 0; tmp < meth_set_size; tmp++)
            {
                vector_add(curr, var_data[j]);
                if (j >= var_list->len)
                    break;
    ...
    if (sharedr == 1)
    {
        print_status("# Methlyations with shared reads to first in set: %i entries\t%s", (int)var_set->len, asctime(time_info));
    }
    else if (sharedr == 2)
    {
        print_status("# Methlyations with shared reads to any in set: %i entries\t%s", (int)var_set->len, asctime(time_info));
    }
    else
    {
        print_status("# Methlyations within %d (max window: %d) bp: %i entries\t%s", distlim, maxdist, (int)var_set->len, asctime(time_info));
    }
    ...
    for (i = 0; i < nthread; i++)
        pthread_create(&tid[i], NULL, meth_pool, w);
    for (i = 0; i < nthread; i++)
...
static char *evaluate(vector_t *var_set)
{
    ...
    { // all, singles
        ...
        // print_status("%s : %d\n",read_data[seti]->name,read_data[seti]->is_reverse);
        vector_add(stats, s);
    }
    if (var_set->len > 1)
    { // doubles and beyond
    ...
    /* todo : assume all CpG sites are meth*/
    else if (METH_EAGLE)
    {
        size_t max_seti = 0;
        double r = stat[0]->mut - stat[0]->ref;
        ...
        for (i = 0; i < stat[max_seti]->combo->len; i++)
        {
            ...
            if (modify_already == 0)
            {
                modify_ref = strdup(refseq);
                modify_ref_length = refseq_length;
                modify_already = 1;
            }
            int altseq_length;
            // print_status("%s\n", modify_ref);
            modify_ref = construct_altseq(modify_ref, modify_ref_length, (vector_int_t *)stat[max_seti]->combo, var_data, &altseq_length);
            modify_ref_length = altseq_length;
        }
        variant_print(&output, v, 0, stat[max_seti]->seen, stat[max_seti]->ref_count, stat[max_seti]->alt_count, log_add_exp(total, stat[max_seti]->ref), has_alt, stat[max_seti]->ref);
        vector_free(v); //variants in var_list so don't destroy
    }
...
vector_t *varMethlist_create(char *modifyRef, vector_t *var_list)
{
    vector_t *methlist = vector_create(8, VARIANT_T);
    print_status("create methlyation list ,the reference length is %ld\n", strlen(modifyRef));
    variant_t **var_data = (variant_t **)var_list->data;
    for (int i = 0; i < strlen(modifyRef); i++)
    {
        if (modifyRef[i] == 'M' || modifyRef[i] == 'W')
        {
            int pos = i + 1;
            char *ref, *chr, *alt;
            chr = var_data[0]->chr;
            if (modifyRef[i] == 'M')
            {
                ref = strdup("C");
                alt = strdup("M");
                variant_t *v = variant_create(chr, pos, ref, alt);
                vector_add(methlist, v);
            }
            else if (modifyRef[i] == 'W')
            {
                ref = strdup("G");
                alt = strdup("W");
                variant_t *v = variant_create(chr, pos, ref, alt);
...
// modify original reference : assume all CpG sites are methylation
void meth_ref_fetch(char *modifyRef)
{
    meth_ref = strdup(modifyRef);
    meth_ref_length = modify_ref_length;
    for (int i = 0; i < modify_ref_length - 1; i++)
    {
        if ((modifyRef[i] == 'C' || modifyRef[i] == 'c') && modifyRef[i + 1] == 'G')
        {
            meth_ref[i] = 'M'; // use 'M' represent forward methylation c
            meth_ref[i + 1] = 'W';
        }
        // if ((modifyRef[i] == 'G' || modifyRef[i] == 'g') && modifyRef[i + 1] == 'C')
        // {
        //     meth_ref[i] = 'W'; // use 'W' represent reverse methylation c
        // }
    }
}
...
int main(int argc, char **argv)
{
    ...
    if (METH_EAGLE)
    {
        // print_status("Original_ref : %s\n\n", or_ref);
        // print_status("Modify : %s\n\n", modify_ref);
        // meth_ref_fetch(modify_ref);
        meth_ref_fetch(modify_ref);
        // print_status("Meth_ref : %s\n", meth_ref);
        print_status("--------------------------------------------------------------\n");
        // vector_t *meth_list = methlist_create(meth_ref);
        vector_t *meth_list = varMethlist_create(meth_ref, var_list);
        out_fh = fopen("MFtest_meth_bwameth.tab", "w");
        pthread_mutex_init(&refseq_lock, NULL);
        meth_process(meth_list, out_fh);
        if (out_file != NULL)
            fclose(out_fh);
        else
            fflush(stdout);
        pthread_mutex_destroy(&refseq_lock);
        print_status("Max var_set : %d\n", max_var_set_len);
    }
    khiter_t k;
